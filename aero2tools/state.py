...
class Isen:
    ...
    def attr_pulse(self):
        ...
        try:
            setattr(self, '_propagating', True)
        
            prop = Propagator()
            prop.enqueue(self)
            prop.run()
        finally:
            setattr(self, '_propagating', False)    
    ...
    def _propagate_step(self, propagator: Propagator):
        ...
        for child, rel in self._children:
            ...
            changed = rel.propagate(forward = True)
        ...
        for parent, rel in self._parent:
            ...
            changed = rel.propagate(forward = False)
    ...
    def __setattr__(self, name: str, value):
        ...
        _internal_var = (
            "_mach", "GAMMA", "UUID",
            "_parent", "_children", "_tracker", "_updating", "_generating", "_propagating",
        ...
        if m: # T(0), P(0), r(0), A(0)
            ...
            if getattr(self, "_generating", False) and (getattr(self, var, None) is None):
                pass
            else:
                changed = True  
    ...
    def __getattr__(self, name):
        ...
        if m:
            ...
            match var:
                ...
                case "P":
                    return self.P0_P / np.pow(k, (self.GAMMA)/(self.GAMMA - 1))
                
                case "r":
                    return self.r0_r / np.pow(k, 1/(self.GAMMA - 1))
        
        if name == "mach_star":
            return self._mach / np.sqrt(self.Tstar_T)
